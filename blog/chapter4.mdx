...
위의 Human과 Humanoid를 속성이 아닌 값으로 생각했을 때 Union과 Intersection을 설명할 수 있다.
...
그러나 이런 상황에서 자바스크립트는 덕 타이핑 언어이기 떄문에 별도의 타입 에러를 뱉지 않는다.
...
이렇게 수정하면 errorArr에 errorType이 포함되게 되고 자바스크립트는 errorCode 999를 보고 에러를 넘겨준다.
...
식별할 수 있는 유니온은 이펙티브 타입스크립트에서 봤던 개념이지만 실무에서 전혀 사용을 못하고 있었는데, Error 예제를 보니
어쩌면 우리 회사의 코드에 적용해 볼 수 있을 것 같다는 생각이 들었다.
